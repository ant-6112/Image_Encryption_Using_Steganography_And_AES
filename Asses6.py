...
def padding(String): 
 ...
 # changing each value to 8 bits 
 str += '{0:08b}'.format(ord(String[i])) 
 bit8_rep = str + "1" 
 n = len(bit8_rep) 
 while len(bit8_rep) % 512 != 448: 
 bit8_rep += "0" 
 # We are appending the 64 bits form of the length of bits=>64+448=512 
...
def sha1_operation(String): 
 ...
 # 4 rounds with 20 steps each and round its own function func defined below 
 for i in range(0, 80): 
 if 0 <= i <= 19: 
 func = (b & c) | ((~b) & d) 
 key = k1 
...
4 
 ...
 func = b ^ c ^ d 
 key = k4 
 # & 0xffffffff is used for 32 bits masking and avoid overflow 
...
class Request_Message(): 
 ...
 def encrypt_the_POMD(self): 
 # passphrase = "passphrase-used-by-user" 
...
7 
 ...
 [file_out.write(x) for x in (encrypted_session_key, 
...
pay\nChoose an operation: ")) 
 ...
 cvv = input("Enter CVV: ") 
 payment.card_details(cardNo, expiry, cvv) 
...
19BCE2347 Rajvi Jasani 
9 
 print("\nGenerating dual signature for verification of the customer")
 order_message = order.generateOrderText() 
 payment_message = payment.generatePayText() 
 obj_req = Request_Message() 
 obj_req.init(payment_message, order_message) 
 obj_req.encrypt_the_POMD() 
 obj_req.digital_Envelope() 
 obj_req.completeRequest() 
 print("\nVerifying the data received by merchant") 
 verify_by_Merchant() 
